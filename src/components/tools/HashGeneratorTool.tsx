...
export default function HashGeneratorTool() {
  ...
  function copyHash(algo: string, hash: string) {
    ...
    <div className="space-y-6">
          ...
          {["MD5", "SHA-1", "SHA-256", "SHA-384", "SHA-512"].map((algo) => (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                >
                  {copied === algo ? <Check size={12} className="text-emerald-500" /> : <Copy size={12} />}
                  {copied === algo ? "Copied" : "Copy"}
                </button>
              </div>
              <p className="text-xs font-mono text-[#18181B] dark:text-[#E4E4E7] break-all leading-relaxed select-all">
                {hashes[algo]}
              </p>
            </div>
          ))}
      ...
      {!input && (
        <div className="text-center py-12 text-[#A1A1AA]">
          ...
          <p className="text-sm">Type or paste text above to generate hashes</p>
        </div>
      )}
    </div>
  );
...
function md5(string: string): string {
  ...
  function hex(x: number[]) { for (let i = 0; i < x.length; i++) x[i] = rhex(x[i] as number) as any; return (x as any).join(""); }
